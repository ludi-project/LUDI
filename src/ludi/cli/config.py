...
class ConfigCLI:
    ...
    def create_parser(self) -> argparse.ArgumentParser:
        ...
        set_parser.add_argument("--default", action="store_true", help="Set as default backend")
    ...
    def show_config(self, validate: bool = False):
        ...
        if self.config_manager._config_path.exists():
            ...
            print("No configuration file found. Run 'ludi config discover --save' to generate one.")

        if validate:
            config = self.config_manager.load_config()
            print("\nValidation Results:")
            print("-" * 20)
            available_backends = self.config_manager.get_available_backends()
            for name in config.keys():
                if name in available_backends:
                    print(f"  {name}: ✓ Valid")
                else:
                    config[name]
                    if name in self.config_manager._providers:
    ...
    def discover_tools(self, save: bool = False):
        ...
        providers = self.config_manager._providers.values()
    ...
    def set_config(
        self, backend: str, path: str = None, enabled: bool = None, default: bool = False
    ):
        config = self.config_manager.load_config()
    ...
    def test_installations(self, backend: str = None):
        ...
        for provider in providers:
            ...
            backend_config = self.config_manager.get_backend_config(provider.backend_name)
            if not backend_config:
                print(f"⊘ {provider.backend_name.upper()}: Not configured")
                continue
            ...
            valid = provider.validate(backend_config.path)
