...
"""
    ...
    def read_pointer(self, addr: int) -> Optional[int]:
        ptr_size = self.ghidra.analyzer.architecture.bits // 8 if hasattr(self.ghidra, 'analyzer') else 8
        data = self.read(addr, ptr_size)
        if data:
            endian = self.ghidra.analyzer.architecture.endian if hasattr(self.ghidra, 'analyzer') else 'little'
            if endian == 'big':
