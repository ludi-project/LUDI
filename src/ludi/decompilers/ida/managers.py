...
class IdaFunctionManager(FunctionManager):
    ...
    def get_basic_blocks(self, addr: int, level: Optional[str] = None) -> List[BasicBlock]:
        ...
        for block in flowchart:
            block_instr_addrs = [ea for ea in instr_addrs if block.start_ea <= ea < block.end_ea]
    ...
    def get_cfg(self, addr: int) -> Optional["CFG"]:
        ...
        basic_blocks = self.get_basic_blocks(addr)
        edges = []
    ...
    def get_representation(self, addr: int, level: str = "disasm") -> Optional[str]:
        if level == "disasm":
            ...
            return self.get_decompiled_code(addr, "pseudocode")
        elif level == "ir":
            return self.get_decompiled_code(addr, "microcode")
        return None
    ...
    def get_string_references(self, addr: int) -> List[tuple]:
        ...
        for ea in self.ida.idautils.FuncItems(func.start_ea):
            for xref in self.ida.idautils.XrefsFrom(ea):
                if xref.type in [self.ida.ida_xref.dr_R, self.ida.ida_xref.dr_O]:  # Data references
                    # Check if the referenced address contains a string
                    string_val = self.ida.idc.get_strlit_contents(xref.to)
                    if string_val:
                        string_refs.append((xref.to, string_val.decode('utf-8', errors='ignore')))
            ...
            for struct_idx in range(self.ida.ida_struct.get_struc_qty()):
                ...
                if struct_id != self.ida.idc.BADADDR:
                    ...
                    if struct:
                        ...
                        types.append(
                            Type(name=name or f"struct_{struct_id:x}", size=size, kind=kind)
                        )
        except Exception:
            pass
        ...
        try:
            for enum_idx in range(self.ida.ida_enum.get_enum_qty()):
                ...
                if enum_id != self.ida.idc.BADADDR:
                    ...
                    types.append(Type(name=name or f"enum_{enum_id:x}", size=size, kind="enum"))
        except Exception:
            pass
...
class IdaBinaryManager(BinaryManager):
    ...
    def get_file_info(self) -> dict:
        try:
            ...
            bits = (
                64
                if self.ida.idc.get_inf_attr(self.ida.idc.INF_LFLAGS) & self.ida.idc.LFLG_64BIT
                else 32
            )
...
class IdaTypeManager(TypeManager):
    ...
    def get_all(self) -> List[Type]:
        types = []
        types.extend(self.get_primitive_types())
        types.extend(self.get_user_types())
        return types

    def get_by_name(self, name: str) -> Optional[Type]:
        for typ in self.get_all():
            if typ.name == name:
                return typ
        return None
...
class IdaArchitectureManager(ArchitectureManager):
    ...
    def endian(self) -> str:
        try:
            return (
                "big"
                if self.ida.idc.get_inf_attr(self.ida.idc.INF_LFLAGS) & self.ida.idc.LFLG_MSF
                else "little"
            )
        except Exception:
            return "little"
