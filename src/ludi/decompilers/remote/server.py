...
class LudiServer:
    def __init__(self):
        ...
        self.sessions: Dict[str, ludi.LUDI] = {}
    ...
    def _setup_routes(self):
        ...
        async def create_session(
            ...
            temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=f"_{binary.filename}")
            ...
            try:
                if backend:
                    analyzer = ludi.LUDI(backend, temp_file.name)
                else:
                    analyzer = ludi.auto(temp_file.name)
        ...
        async def list_sessions():
            return {
                session_id: {"backend": analyzer.backend_name, "binary_path": analyzer.binary_path}
                for session_id, analyzer in self.sessions.items()
            }

        @self.app.post("/sessions/{session_id}/{manager}/{method}")
        async def call_method(session_id: str, manager: str, method: str, call: MethodCall):
            ...
            if not hasattr(analyzer, manager):
                raise HTTPException(status_code=400, detail=f"Unknown manager: {manager}")
