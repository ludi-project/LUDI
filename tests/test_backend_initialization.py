...
from ludi.ludi import LUDI, SUPPORTED_BACKENDS
...
class TestBackendInitialization:
    ...
    def test_ludi_load_binary(self):
        ...
        try:
            analyzer = LUDI("angr", "/bin/ls")
            print(f"  angr: SUCCESS - found {len(analyzer.functions.get_all())} functions")
            analyzer.close()
        except Exception as e:
        ...
        try:
            analyzer = LUDI("auto", "/bin/ls")
            backend_type = type(analyzer._decompiler).__name__
